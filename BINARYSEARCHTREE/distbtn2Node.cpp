...
public:
 ...
 int findLevel (Node* root, Node* target, int level) {
     ...
     if (root -> data < target -> data) return findLevel (root -> right, target, level + 1);
     else return findLevel (root -> left, target, level + 1);
...
Node* findLCA (Node* root, int a, int b) {
  ...
  if (root -> data == a || root -> data == b) return root; 
		...
		if (exists(root ->left, aRoot) && exists (root -> left, bRoot)) return findLCA(root -> right, a, b);
}
...
int main() {
    ...
    cout << findDistance(root, 3, 9) << endl;
